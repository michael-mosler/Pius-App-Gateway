...
class CloudantDb {
    ...
    connect() {
        try {
            ...
            const verror = new Verror({
                name: ErrorCodes.DatabaseDriverError.type,
                cause: err,
                info: {
                    errno: ErrorCodes.DatabaseDriverError.errno,
                    dbName: this.name,
                },
    ...
    /**
     * Copies this database to database named <i>to</i>. Copying is done using Cloudant replication.
     * If target does not exist it will be created.
     * <b>Note</b><br/>
     * Replication is a completely asynchronous process. Thus, we can only check if replication was
     * started successfully but we will not know about any problems that might occur during the actual
     * replication process.
     * @param {String} to - Name of target, if omitted it defaults to <i>${name}_bak</i>.
     * @returns {Promise<*|Verror>} - Resolves when replication was submitted successfully.
     */
    copy({ to = null } = {}) {
        return new Promise((resolve, reject) => {
            try {
                const target = to || `${this.name}_bak`;
                const replication = new Replication();
                replication.readReplicationDocumentTemplate()
                    .then(replicationDocTemplate => this.configureReplicationDocumentTemplate(replicationDocTemplate, this.name, target))
                    .then(replicationDocument => replication.execute(replicationDocument))
                    .then(() => resolve())
                    .catch((err) => {
                        /* istanbul ignore if */
